...
const AddCustomerPage: React.FC<AddCustomerPageProps> = ({
  ...
  const handleSave = async () => {
    console.log('[AddCustomer] Save button clicked');
    
    if (!validateFields()) {
      console.log('[AddCustomer] Validation failed');
      return;
    }
    ...
    try {
      ...
      console.log('[AddCustomer] Creating customer:', payload);
      console.log('[AddCustomer] Calling customerService.createCustomer...');

      const customer = await customerService.createCustomer(payload);
      
      console.log('[AddCustomer] Customer created successfully:', customer);
      ...
      console.error('[AddCustomer] Error message:', error?.message);
      console.error('[AddCustomer] Error stack:', error?.stack);
      Alert.alert(
        'Error', 
        error?.message || error?.toString() || 'Failed to save customer. Check console for details.'
      );
    }
  };
